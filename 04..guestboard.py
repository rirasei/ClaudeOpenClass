...
# Vercel's deployment filesystem is read-only, but /tmp is writable within a
# running instance. Locally (no VERCEL env var) we keep using the project
# folder so the data persists across restarts during development.
_DATA_DIR = "/tmp" if os.environ.get("VERCEL") else os.path.dirname(os.path.abspath(__file__))
DATA_FILE = os.path.join(_DATA_DIR, "guestboard.json")
